    ...
    public function detect($content){
        ...
        while (Tea::allowRetry($_runtime["retry"], $_retryTimes, $_now)) {
            ...
            try {
                ...
                $respMap = $this->_readAsJson($_response, "alipay.security.risk.content.detect");
                if ($this->_isCertMode()) {
                    if ($this->_verify($respMap, $this->_extractAlipayPublicKey($this->_getAlipayCertSN($respMap)))) {
                        return Model::toModel($this->_toRespModel($respMap), new AlipaySecurityRiskContentDetectResponse());
                    }
                }
                else {
