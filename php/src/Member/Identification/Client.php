...
class Client extends BaseClient{
    ...
    public function certify($certifyId){
        ...
        $response = [
            "body" => $this->_generatePage("GET", $systemParams, $bizParams, $textParams, $sign)
            ];
        return Model::toModel($response, new AlipayUserCertifyOpenCertifyResponse());
    }
            ...
            try {
                ...
                if ($this->_isCertMode()) {
                    if ($this->_verify($respMap, $this->_extractAlipayPublicKey($this->_getAlipayCertSN($respMap)))) {
                        return Model::toModel($this->_toRespModel($respMap), new AlipayUserCertifyOpenInitializeResponse());
                    }
                }
                else {
    ...
    public function query($certifyId){
        ...
        while (Tea::allowRetry($_runtime["retry"], $_retryTimes, $_now)) {
            ...
            try {
                ...
                if ($this->_isCertMode()) {
                    if ($this->_verify($respMap, $this->_extractAlipayPublicKey($this->_getAlipayCertSN($respMap)))) {
                        return Model::toModel($this->_toRespModel($respMap), new AlipayUserCertifyOpenQueryResponse());
                    }
                }
                else {
                    if ($this->_verify($respMap, $this->_getConfig("alipayPublicKey"))) {
