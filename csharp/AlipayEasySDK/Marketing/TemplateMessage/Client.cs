        ...
        public AlipayOpenAppMiniTemplatemessageSendResponse Send(string toUserId, string formId, string userTemplateId, string page, string data)
        {
            ...
            while (TeaCore.AllowRetry((IDictionary) runtime_["retry"], _retryTimes, _now))
            {
                ...
                try
                {
                    ...
                    Dictionary<string, object> respMap = _readAsJson(response_, "alipay.open.app.mini.templatemessage.send");
                    if (_isCertMode())
                    {
                        if (_verify(respMap, _extractAlipayPublicKey(_getAlipayCertSN(respMap))))
                        {
                            return TeaModel.ToObject<AlipayOpenAppMiniTemplatemessageSendResponse>(_toRespModel(respMap));
                        }
                    }
                    else
                    {
