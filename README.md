...
* Alipay Easy SDK和Alipay SDK并无冲突，可以共存。
	* 如果您所对接的能力和场景，Alipay Easy SDK已完全支持（[已支持的API列表](#apiList)），或者可以通过[通用接口](https://github.com/alipay/alipay-easysdk/blob/master/APIDoc.md#generic)覆盖，您也可以去除对Alipay SDK的依赖。
